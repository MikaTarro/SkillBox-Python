...
def one_year_of_war():
    ...
    tank = Tank("Танк Тайкус")
    attacker = Attacker("дд Квайгон")
    second_attacker = Attacker("дд Обиван")
    healer = Healer("хил Тралл")
    second_healer = Healer("хил Андуин")
    ...
    evil_names = ["Гулдан", "Дарт Мол", "Саурон", "Палпатин", "Гривус", "Керриган"]
    mob_warrior = MonsterBerserk("Sage " + random.choice(evil_names))
    mob_ranger = MonsterHunter("meleDD " + random.choice(evil_names))
    ...
    for day in range(1, 366):
        ...
        if day % 2 == 0 and len(evil_team) < 4:
            newborn_evils = [MonsterBerserk("Sage " + random.choice(evil_names)),
                             MonsterHunter("meleDD " + random.choice(evil_names))]
